...
[s, fs] = audioread('speechfiles/H.22.16k.wav');
...
for i = 1:1:Nfr
    frame = s(i)*win % a frame of speech windowed by the Hamming window
    energy(i) = sum(frame.^2) % calculate energy
    ZCr(i) = 0.5*sum(abs(sign(frame(2:end))-sign(frame(1:end-1)))) % calculate zero crossings
...
VUSi = interp1(T, VUS, 0:1/fs:length(s)/fs-1/fs);
