...
def SinM_FrameToFramePeakMatching(SinM_prev: SinMFrame, SinM_curr: SinMFrame, S, Fs, Delta=10):
    ...
    for n in range(L_1):
        ...
        candidates = np.where(np.abs(F_2 - F_1[n]) < Delta)[0]
        if len(candidates) == 0:
            continue  # dead
        # closest-frequency candidate
        j = candidates[np.argmin(np.abs(F_2[candidates] - F_1[n]))]
        # has no better match to the remaining unmatched frequencies of frame k
        # equation 27
        closest_prev = np.argmin(np.abs(F_1 - F_2[j]))
        if closest_prev == n:
            I_1[n] = j
            I_2[j] = n
